...
| | |
|---|---|
| 对弈 | 人机 · 执红/执黑 · **点击走棋** · 悔棋 · 翻转 |
| AI | 迭代加深 · α-β · 置换表 · 静默搜索 |
| 辅助 | 棋盘提示箭头 · 局面分 · 被吃子 · 着法 |
| 体验 | 暗色 UI · **中文语音播报** · 自动存档 · 快捷键 |

语音用系统 **Web Speech API**（`zh-CN`），播报选子、着法、吃子、将军、胜负；需系统装有中文语音包。

快捷键：`N` 新局 · `U`/`Z` 悔棋 · `F` 翻转 · `H` 提示 · `M` 音效 · `Esc` 取消选子
